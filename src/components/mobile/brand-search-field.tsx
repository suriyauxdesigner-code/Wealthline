...
import { BrandLogo } from "@/components/finance/merchant-icon";
...
interface MobileBrandSearchFieldProps {
  label: string;
  placeholder?: string;
  value: string;
  ...
  pastNames: string[];
  /** Already-resolved leading icon — callers decide their own fallback chain (category icon, account-group icon, etc.). */
  icon: React.ReactNode;
}

// wl-mobile field-row chrome shared by any free-text field that benefits
// from brand search (merchant, account institution, …): a live icon
// preview plus a suggestion dropdown (the caller's own past values first, a
// debounced Brandfetch search appended). Typing without picking a
// suggestion works exactly like a plain text field.
export function MobileBrandSearchField({
  label,
  placeholder,
  value,
  onChange,
  ...
  pastNames,
  icon,
}: MobileBrandSearchFieldProps) {
  const [open, setOpen] = React.useState(false);
  const { suggestions } = useMerchantSuggestions(value, pastNames);
